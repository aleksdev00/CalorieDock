...
export default async function EditMealPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="mx-auto w-full max-w-2xl flex-1 px-4 py-10 sm:px-6"><Button asChild variant="ghost"><Link href={`/meals/${id}`}>← Back to meal</Link></Button><section className="mt-6 rounded-2xl border bg-card p-6 shadow-sm sm:p-8"><h1 className="text-3xl font-bold tracking-tight">Edit meal</h1><div className="mt-8"><MealForm mealId={id} consumedAtInstant={result.data.consumed_at} defaultValues={{ name: result.data.name, mealType: result.data.meal_type, consumedAt: toLocalDateTime(result.data.consumed_at) }} /></div></section></main>
}

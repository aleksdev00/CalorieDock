...
import type { WeightEntry, WeightPageData, WeightUnit } from "../types"
...
async function context() {
  ...
  if (error || !user) return { supabase, user: null, profile: null, weightUnit: null }
  ...
  return { supabase, user, profile: profileResult.data as { goal: ProfileGoal | null } | null, weightUnit: preferenceResult.data?.weight_unit as WeightUnit | undefined }
}

function canonicalWrite(input: ValidatedWeightEntryInput, weightUnit: WeightUnit): ActionResult<{ weight_kg: number; recorded_at: string; note: string | null }> {
  const weightKg = displayToKilograms(input.weight, weightUnit)
  if (weightKg < 20 || weightKg > 500) return { success: false, error: { code: "VALIDATION_ERROR", message: weightUnit === "lbs" ? "Weight must be between 44.1 and 1102.3 lb." : "Weight must be between 20 and 500 kg.", fieldErrors: { weight: [weightUnit === "lbs" ? "Enter a weight between 44.1 and 1102.3 lb." : "Enter a weight between 20 and 500 kg."] } } }
...
export async function getWeightPageData(): Promise<ActionResult<WeightPageData>> {
  const { supabase, user, profile, weightUnit } = await context()
  if (!user) return { success: false, error: AUTH_ERROR }
  if (!profile || !weightUnit) return { success: false, error: LOAD_ERROR }
  ...
  return { success: true, data: { entries: calculated.history, progress: calculated.progress, weightUnit, goal: profile.goal } }
}

export async function getWeightEntry(id: string): Promise<ActionResult<{ entry: WeightEntry; weightUnit: WeightUnit }>> {
  const { supabase, user, profile, weightUnit } = await context()
  if (!user) return { success: false, error: AUTH_ERROR }
  if (!profile || !weightUnit) return { success: false, error: LOAD_ERROR }
  const { data, error } = await supabase.from("weight_entries").select(COLUMNS).eq("id", id).eq("user_id", user.id).maybeSingle()
  if (error) return { success: false, error: LOAD_ERROR }
  return data ? { success: true, data: { entry: data as WeightEntry, weightUnit } } : { success: false, error: NOT_FOUND }
}

export async function createWeightEntry(input: ValidatedWeightEntryInput): Promise<ActionResult<WeightEntry>> {
  const { supabase, user, profile, weightUnit } = await context()
  if (!user) return { success: false, error: AUTH_ERROR }
  if (!profile || !weightUnit) return { success: false, error: SAVE_ERROR }
  const write = canonicalWrite(input, weightUnit)
